...
class_name VelocityGradientShotModule

@export_range(0.1, 4.0, 0.01) var minimum_multiplier: float = 0.75
@export_range(0.1, 4.0, 0.01) var maximum_multiplier: float = 1.25


func _init() -> void:
	module_id = &"velocity_gradient"
...
func transform(shots: Array[ShotSpec]) -> Array[ShotSpec]:
	...
	for shot_index in range(valid_shots.size()):
		...
		output.append(shot.with_speed(shot.speed * speed_multiplier, module_id))
	return output
